...
class CampaignSchedulingOptimizer {
  ...
  def optimize(config: OptimizationConfig = OptimizationConfig()): OptimizationResults = {
    ...
    println(s"Configuration: $finalConfig")
    println("=" * 50)
    ...
    try {
      ...
      val customers = loadCustomerData(finalConfig, spark)
      ...
      val problem = new CampaignSchedulingProblem(
        customers = customers,
        maxCustomersPerHour = finalConfig.maxCustomersPerHour,
        campaignBudget = finalConfig.campaignBudget
      )
      ...
      val algorithm = createNSGAII(problem, finalConfig, sparkContext)
      
      // Step 4: Run optimization
      println("\nStep 4: Running optimization...")
      println(s"Population size: ${finalConfig.populationSize}")
      println(s"Max evaluations: ${finalConfig.maxEvaluations}")
  ...
  def optimizeForZeppelin(
    ...
    val config = OptimizationConfig(
      ...
      sparkMaster = None // Auto-detect based on environment
    )
